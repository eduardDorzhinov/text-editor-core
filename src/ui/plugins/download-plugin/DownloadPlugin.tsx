...
import { insertDownload } from "./insertDownload";

import styles from "./DownloadPlugin.module.scss";
...
const ALL_SOURCE_OPTIONS: SourceOption[] = [{
  id: "url",
  settingKey: "downloadUrl",
  label: "По ссылке",
  Icon: FiLink,
}, {
  id: "file",
  settingKey: "downloadUpload",
  label: "Загрузить",
  Icon: FiUploadCloud,
...
export const InsertDownloadDialog = ({
  activeEditor,
  onClose,
}: {
  ...
  const onSave = (fileSrc: string, name: string) => {
    activeEditor.update(() => {
      insertDownload(fileSrc, name || "Файл для скачивания");
    });
    onClose();
  };
...
export const DownloadPlugin = (): ReactElement | null => {
  const [ editor ] = useLexicalComposerContext();
  useEffect(() => {
    if (!editor.hasNodes([ DownloadNode ])) {
      throw new Error("DownloadPlugin: DownloadNode is not registered on editor");
    }
  }, [ editor ]);
