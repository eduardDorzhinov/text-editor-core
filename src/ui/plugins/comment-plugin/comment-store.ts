...
export type Thread = {
  ...
  quote: string,
  type: "thread",
  // Для комментариев ко ВСЕЙ ноде: id блока (block-anchor id, стабильный и
  // персистится). Если задан — обсуждение привязано к блоку целиком, а не к
  // выделенному тексту (MarkNode отсутствует). Для inline-комментариев undefined.
  blockId?: string,
};
...
export class CommentStore {
    ...
    if (thread !== undefined && commentOrThread.type === "comment") {
      for (let i = 0; i < nextComments.length; i++) {
        ...
        if (comment.type === "thread" && comment.id === thread.id) {
          ...
          newThread.comments.splice(
            insertOffset, 0, commentOrThread,
          );
          break;
        }
      }
  ...
  _withRemoteTransaction(fn: () => void): void {
    ...
    if (provider !== null) {
      const doc = (provider as unknown as { doc: Doc }).doc;
  ...
  _createCollabSharedMap(commentOrThread: Comment | Thread): YMap<any> {
    ...
    if (type === "comment") {
      ...
      if (commentOrThread.blockId !== undefined) {
        sharedMap.set("blockId", commentOrThread.blockId);
      }
  ...
  registerCollaboration(provider: Provider): () => void {
    ...
    // The YJS types explicitly use `any` as well.
    const onSharedCommentChanges = (events: Array<YEvent<any>>,
            ...
            for (let s = 0; s < deltas.length; s++) {
              ...
              if (Array.isArray(insert)) {
                ...
                for (let d = 0; d < del; d++) {
                  ...
                  this._withLocalTransaction(() => {
                    this.deleteCommentOrThread(commentOrThread,
                      parentThread as Thread);
                  });
                  offset++;
                }
