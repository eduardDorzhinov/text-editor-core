...
function $convertLayoutContainerElement(domNode: HTMLElement): DOMConversionOutput | null {
  // Сначала inline-style (надёжно для clipboard-импорта). При вставке
  // из буфера DOM detached → getComputedStyle вернёт пустую строку,
  // и колонка распадётся в обычный div.
  const inline = domNode.style.gridTemplateColumns;
  const computed = inline ||
    window.getComputedStyle(domNode).getPropertyValue("grid-template-columns");
  const node = $createLayoutContainerNode(computed || "");
  return { node };
}
