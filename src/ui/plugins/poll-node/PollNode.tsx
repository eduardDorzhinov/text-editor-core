...
export class PollNode extends DecoratorNode<ReactElement> {
  $config() {
    return this.config("poll", {
      ...
      stateConfigs: [{ flat: true, stateConfig: questionState }, { flat: true, stateConfig: optionsState }],
    });
  }
  ...
  setQuestion(valueOrUpdater: StateValueOrUpdater<typeof questionState>): this {
    return $setState(
      this, questionState, valueOrUpdater,
    );
  }
  ...
  setOptions(valueOrUpdater: StateValueOrUpdater<typeof optionsState>): this {
    return $setState(
      this, optionsState, valueOrUpdater,
    );
  }
  ...
  toggleVote(option: Option, username: string): this {
    return this.setOptions((prevOptions) => {
      ...
      const clonedOption = cloneOption(
        option, option.text, votesClone,
      );
      const options = Array.from(prevOptions);
      options[ index ] = clonedOption;
      return options;
  ...
  exportDOM(): DOMExportOutput {
    ...
    element.setAttribute("data-lexical-poll-options",
...
export function $createPollNode(question: string, options: Options): PollNode {
  return new PollNode().setQuestion(question)
    .setOptions(options);
}
