...
import styles from "./PdfToolbarPlugin.module.scss";
...
export const InsertPdfDialog = ({
  ...
  const onSave = (file: string) => {
    activeEditor.update(() => {
      insertPdf(file, title || "PDF");
    });
    onClose();
  };
...
export const PdfPlugin = (): ReactElement | null => {
  const [ editor ] = useLexicalComposerContext();
  useEffect(() => {
    if (!editor.hasNodes([ PdfNode ])) {
      throw new Error("PdfPlugin: PdfNode is not registered on editor");
    }
  }, [ editor ]);
