      ...
      editor.registerCommand(
        ...
        () => {
          ...
          if ($isRangeSelection(selection)) {
            const titleNode = $findMatchingParent(selection.anchor.getNode(),
            ...
            if ($isCollapsibleTitleNode(titleNode)) {
              ...
              if (container && $isCollapsibleContainerNode(container)) {
                // В редакторе контент всегда виден — открывать (toggleOpen)
                // не нужно, иначе это меняло бы preview-состояние __open.
                // Просто переносим каретку из заголовка в контент.
      ...
      editor.registerCommand(
        ...
        () => {
          editor.update(() => {
            ...
            // false — по умолчанию свёрнут В ПРЕВЬЮ (скрываемый текст).
            // В редакторе всё равно виден (createDOM рендерит открытым).
            $insertNodeToNearestRoot($createCollapsibleContainerNode(false).append(title.append(paragraph),
